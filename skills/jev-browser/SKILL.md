...
If JEV tools are absent from this session's tool catalog, report that connection problem explicitly. Do not claim JEV is operating through another browser tool or silently launch a separate server and use a different tab. Verify the installed package with `node scripts/check-install.mjs` and Codex inventory with `node scripts/check-codex-mcp.mjs` from the package directory when available. The package offers `node scripts/register-mcp.mjs` for direct local MCP registration. Restart the MCP connection or use a new task after registration. A standalone CLI/dashboard is appropriate only when explicitly chosen, using its own pairing link.
