...
server.registerTool('jev_run',{description:'Start an autonomous browser task with a complete goal, adaptive plan, exact field values and outcome checks. browser=isolated opens url; browser=extension uses the explicitly shared Chrome tab at its current page (url is informational in that mode). Explicitly supplied passwords/codes belong in secrets [{label,text,origin}], never ordinary values. Secret text is held only in process memory, not sent to JEV or persisted; origin binds it to the exact site. Check jev_status first. Returns immediately; inspect with jev_task.',inputSchema:TaskRequest.shape},async input=>{const t=await manager.start(input);return result(summary(t.id));});
